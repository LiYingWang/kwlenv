...
For animal classes, mammals are the primary dietary source throughout all layers, with fish as the secondary source in the layer 4 and birds in the layer 3 and the layer 2 (@fig-NISP-proportion: A). Fish and birds both identified from the layer 5 to the layer 3; however, their proportions (less then 10%) are much lower compared to mammals. Reptile is represented by one piece of turtle shell found in the layer 4, which might not be a common food content at the site. Within those identifiable mammals, deer is the major source from the layer 6 to the layer 2 that account for at least 85 % per layer, while the layer 1 is predominated only by cattle (@fig-NISP-proportion: B). Wild boar was present from the layer 6 to layer 3, and has a higher proportion in the layer 4 (`r CL4_boar` %), while muntjac appeared in the layer 5, and continuously present to the layer 2. Given that relative abundances could be impacted by sample size effect across layers, the relationship between NISP and sample size for major taxa were examined by linear regression model. The results shows that  
