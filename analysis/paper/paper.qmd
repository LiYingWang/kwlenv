...
Given that deerskin was a primary commodity in local regional exchange since the 14th century and was in high demand during the Dutch colonization period [@Cheng2017], the frequency of skeletal elements and surface modifications such as cutmarks of deer bones were examined to assess butchery and transport strategy. This can also shed light on how indigenous people utilize deer in response to high economic demand when involved in regional trade network. Skeletal elements was quantified using NISP and classified into groups based on meat yield: meaty (e.g., upper forelimb and upper hindlimb ), mid-meaty (e.g., body parts), and less meaty portions (e.g., skull and lower limb) [@Madrigal2004; @Munro2004]. Teeth without associated cranial elements were excluded here to reduce overrepresentation due to their higher preservation compared to othe parts. While lower limbs and skull bones have minimal meat, they are often valued in many cultures for marrow and soft tissues, such as tongue and brain [@Madrigal2002]. Thus, less meaty elements were considered as secondary source of nutrition in the interpretation. Similarly, the degree of fragmentation could also imply the extraction of marrow or grease. The degree of fragmentation is calculated by dividing Minimal Number of Elements (MNE) by NISP. For butcher patterns, cut marks on major limb joints, including the shoulder, elbow, wrist, hip, knee, and ankle [@Lyman2005], were identified and quantified for a comparison between different temporal sequences. In addition, modifications such as cut marks, fracturing, and burning were examined, especially the cut marks associated with the skinning process. Skinning processes can be identified by the encircling and straight marks left on the lower limb, such as metapodials, which result from the series of cuts typically made to acquire intact animal hides [@Costamagno2019; @Isaakidou2007].
...
fauna_combined_context_all <-
  ...
  mutate(category = fct_relevel(category, "deer","boar","muntjac","cattle","rat","bird","fish","turtle")) %>%  mutate(modify = case_when(str_detect(`人為痕跡`, "切")|str_detect(`人為痕跡`, "削")|
                            str_detect(`人為痕跡`, "砍")~ "cutmarks",
                            str_detect(`人為痕跡`, "火")|str_detect(`人為痕跡`, "燒")~ "burn", TRUE ~ "no")) %>%filter(is.na(refitted)) # for NISP
...
deer_NISP_per <- round(deer_NISP/(total_NISP-unident), 3)*100
```

The NISP of faunal assemblages from Kiwulan analyzed in this study is `r total_NISP`, where deer family is the majority that accounts for about `r deer_NISP_per`% of the identified bones, followed by boar and cattle as the second and third taxon (@tbl-counts). Within the assemblages from the deer family, sika deer is the most common animal, with a few assemblages identified as muntjac and a specimen indicating a possible affiliation with sambar deer. Although sika deer was the major animal source at the site, variations in animal use could be observed between different cultural layers across 600 years. To highlight general patterns of dietary practices in terms of their economic values over time, the deer family were separated into two categories based on body size, deer (sika deer, sambar deer, and large deer) and muntjac, while water buffalo and yellow cattle were combined into one category, cattle. Rats (Rattus sp.) were excluded from further analysis as they are normally considered commensal animals, and their relatively intact skeletons from one single archaeological context show no direct evidence of being part of the daily diet.
...
For animal classes, the relative abundance shows that mammals are the primary dietary source throughout all layers, with fish as the secondary source in CL4 and birds in CL3 and CL2 (@fig-NISP-proportion: A). Fish and birds are both identified from CL5 to CL3; however, their proportions (less than 10%) are much lower compared to mammals. The reptile is represented by one piece of turtle shell found in CL4, which might not be a common food content at the site. Within those identifiable mammals, deer is the major source from CL6 to CL2 that accounts for at least 85% per layer, while CL1 is predominated only by cattle (@fig-NISP-proportion: B). Wild boar was present from CL6 to CL3, and had a higher proportion in CL4 (`r CL4_boar` %), while muntjac appeared in CL5, and was continuously present until CL2. In general, there is an increase in animal diversity in CL4 and CL5, especially in CL4, where boar and fish have slightly higher proportions compared with other layers. Given that the taxonomic richness, or animal diversity, could be impacted by sample size varied from layer to layer, the relationship between the number of taxa and sample size (NISP) was examined by linear regression model. The result shows no correlation between the diversity of animals and sample size ($R^2$ = `r div_lm[1]`, P = `r div_lm[2]`), which rules out the potential sample size bias.

```{r}
#| label: fig-NISP-proportion
#| echo: false
#| fig-cap: "A: Relative abundance of vertebrate classes by cultural layers based on normed NISP. The data is derived from Table 1. B: Relative abundance of mammals by cultural layers based on NISP. CL6 to CL1 prepresent cultural layers from the earliest to the latest layer. The NISP for each layer is CL6: 46, CL5: 65, CL4: 156, CL3: 28, CL2: 12, and CL1: 25."
...
Since deer remain was the major component of faunal assemblage at Kiwulan according to their high quantity and ubiquity, the frequency of skeletal elements was examined to explore any changes in processing decisions of meat portions between different phases. Deer bones were grouped into five body portions based on their nutritional value [@Madrigal2004], including head (cranium, mandible), axial (vertebrae, sternum, ribs), upper forelimb (scapula, humerus, ulna, and radius), upper hindlimb (femur, tibia, innominate), and lower limb (carpals, metacarpals, tarsals, metatarsals, astragalus, calcaneus, and phalanges). The result (@fig-deer-portion: A) shows that the meaty portion, including upper forelimb and upper hindlimb, especially upper forelimb, is the most frequent part throughout the cultural layers, followed by the less-meaty portions, such as lower limb and foot. The consistency of higher numbers of upper limb and lower limb suggests that those elements should be transported together (e.g., @Stiner1991), or marrows from upper limb were commonly utilized. It should be noted that the frequencies of axial elements, such as ribs, were generally low across different phases, likely due to a high degree of fragmentation, butchery practices, or dietary preferences. A different pattern is observed in the frequency of head bones, which were a few in CL6, increased to the highest number in CL4, and then decreased thereafter. 

Considering varying frequencies of skeletal parts might be mediated by differential bone density, the relationship between the survivorship of deer skeletal elements and bone mineral density was tested by Pearson correlation. The values of bone survivorship were collected from @Lyman1994's research on deer, and calculated by dividing NISP by expected NISP, followed by multiplying 100%. The result (@fig-deer-portion: B) shows that there is no significant positive correlation ($r$ = `r survivor_cor[1]`, P = `r survivor_cor[2]`)) between skeletal part survivorship and bone density, suggesting that density-mediated bias has little impact on the taphonomic processes of deer bones.
