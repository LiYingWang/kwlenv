...
fauna_combined_taxa <-
  ...
  mutate(group = case_when(str_detect(`Common name`, "deer") ~ 1, str_detect(`Common name`, "Deer") ~ 2,
                           ...
                           str_detect(`Common name`, "fish") ~ 7, str_detect(taxa, "Geo") ~ 8,  
  ...
  bind_rows(summarise(head(., 13), across(where(is.numeric), sum), 
                      across(where(is.character), ~"Total determinate"))) %>%
  bind_rows(summarise(.[14:16, ], across(where(is.numeric), sum), 
                      across(where(is.character), ~"Total indeterminate"))) %>%
